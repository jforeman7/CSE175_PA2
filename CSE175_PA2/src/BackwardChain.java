...
//Jeff Foreman 100216478 Thurs Nov 10 2016
//Made in collaboration with Tim Kim.
//He helped me understand the use of the methods in the classes provided by Professor Noelle.
//Also, he gave the idea that the central Unify Terms can be used to do the bulk of the unify work, such as the Unify Literal and Unify Functions.
//The central unify can be thought of as a tree. Entering Unify Terms starts you in one of three "states", whether or not the first term is a c,v, or f. 
//For each instance, the second term can be one of the three instances: c, v, or f. 
//Thus the central unify is composed of conditional checks based on the relationship between t1 and t2.

//The repetitive use of returning null helps prevent bugs, and helps maintain a clean format of code.
...
public class BackwardChain {
    ...
    //Unify Terms(Variables, Constants, and Functions)
    public BindingList unify(Term t1, Term t2, BindingList bl) 
    {
    	...
    	else if(t1.c != null) //The first term is a constant, now to deal with the variations of t2.
    	{
    		...
    		{
    			return unify(t2, t1, bl); //Tim mentioned that simply recalling unify with the terms flipped will prevent duplicated coding.
    		}
    		...
    		else if(t2.c != null) //The second term is a constant. C vs C.
    		{
    			if(t1.equals(t2)) //Use terms equals method to compare the constants, and if equal return the BL.
    				return new BindingList(bl); //According to the TA, allocating a new memory space is necessary.
    			else
    	...
    	else if(t1.f != null)//The first term is a function, now to deal with the variations of t2.
    	{
    		...
    		{
    			return unify(t2,t1,bl); //Again, using Tim's advice, recalling and switching the terms skips repetitive coding.
    		}
    		else if(t2.c != null) // The second term is a constant.
    		{
